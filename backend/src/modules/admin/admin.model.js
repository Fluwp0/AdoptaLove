...
async function findPetModificationRequests({ limit, offset, search = '' }) {
  ...
  const [rows] = await db.query(
    `SELECT
      ...
      ${buildAgeYearsSql('m', 'mascota_edad_anios_actual')},
      ${buildAgeMonthsSql('m', 'mascota_edad_meses_actual')},
      m.fecha_nacimiento_estimada AS mascota_fecha_nacimiento_estimada_actual,
  ...
  try {
    ...
    await connection.query(
      ...
      [
        ...
        proposed.fecha_nacimiento_estimada ?? request.mascota_fecha_nacimiento_estimada_actual,
