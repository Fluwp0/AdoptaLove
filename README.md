...
- Mantener actualizados `frontend/.env.example` y `backend/.env.example` cuando se agreguen nuevas variables.
- No subir credenciales reales, claves de OpenAI ni configuraciones privadas.
- Si se modifica la base de datos, actualizar `schema.sql` y agregar la migración correspondiente.
- Si se agregan nuevas páginas o rutas, actualizar este README.